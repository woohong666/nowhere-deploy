...
#
# Use install.sh instead if you would rather download the official prebuilt
# Release; that path takes a minute instead of an hour, and trusts upstream's
# published binary (checksum-verified) rather than compiling locally.
#
set -Eeuo pipefail
umask 077
...
Usage:
  sudo bash install-source.sh install --key KEY [--port 2077] --tls 2 \
    --cert /path/fullchain.pem --tls-key /path/privkey.pem
  sudo bash install-source.sh upgrade [--version v1.8.3] [--commit SHA]
  sudo bash install-source.sh rollback
  sudo bash install-source.sh status
  sudo bash install-source.sh link
  sudo bash install-source.sh logs
  sudo bash install-source.sh restart
  sudo bash install-source.sh clean-build
  sudo bash install-source.sh uninstall [--purge]

TLS helper:
  sudo bash install-source.sh prepare-tls --cert /path/fullchain.pem --tls-key /path/privkey.pem
...
Trust boundary:
  ...
  This is the slow, high-assurance path. install.sh downloads the official
  prebuilt Release instead and finishes in about a minute.
EOF
}
...
check_disk_space() {
  ...
  (( avail_src >= 5120 )) ||
    die "Building needs roughly 5GB free under $(dirname "$SRC_DIR"); only ${avail_src}MB available. Free space first, or run 'install-source.sh clean-build' afterwards to reclaim the build tree."
...
check_certificate_access() {
  ...
  runuser -u "$RUN_USER" -- test -r "$CERT" ||
    die "Service user ${RUN_USER} cannot read certificate: ${CERT}
       Copy it with 'install-source.sh prepare-tls --cert ... --tls-key ...' and use the paths it prints."
...
install_or_upgrade() {
  ...
  if [[ -n "$portal" ]]; then
    ...
    warn "Could not read NOWHERE_PORTAL from ${CONFIG_FILE}; run 'install-source.sh link' to inspect it."
  fi
}
