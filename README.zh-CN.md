...
脚本名称统一为 `nowhere-v1.sh`。旧命令 `nowhere.sh` 和 `nowhere-v1-stable.sh` 已不再作为部署入口。
...
> - 手动信任该证书，或
> - 使用 `sudo bash nowhere-v1.sh fingerprint` 查看指纹并固定
...
如果使用 Certbot 维护证书，在 `/etc/letsencrypt/renewal-hooks/deploy/nowhere-v1.sh` 写入更新钩子：
...
赋予权限：`chmod +x /etc/letsencrypt/renewal-hooks/deploy/nowhere-v1.sh`。每次证书续期后将自动完成授权与服务热重载。
...
```bash
# 按 V1 稳定通道重新部署固定版本 v1.8.3
sudo bash nowhere-v1.sh upgrade --version v1.8.3
```

升级时将保留 `/etc/nowhere/url.conf` 配置不变。如果新版本启动检测失败，脚本会自动切回旧版本，防止服务失联。
...
以下为 `nowhere-v1.sh` 的参数（自动化专用 `install.sh` / `install-source.sh` 接受其中一部分：`--version` `--libc` `--key` `--port` `--net` `--tls` `--cert` `--tls-key` `--listen-host`；`install-source.sh` 额外支持 `--commit` `--jobs` `--swap` `--keep-source` `--git-url`）：
...
| `--tls MODE` | `1` | TLS 模式：`1`（临时自签证书）、`2`（本地 PEM 证书文件）。**生产环境必须用 `2`** |
| `--cert PATH` | 无 | 证书全链路径（`fullchain.pem`），TLS 2 必填 |
| `--tls-key PATH` | 无 | 私钥文件路径（`privkey.pem`），TLS 2 必填 |
| `--version TAG` | `v1.8.3` | V1 稳定脚本固定为 `v1.8.3`；`latest`、V2 及其它版本会被拒绝 |
...
界面语言在首次运行时交互选择，或通过环境变量 `NOWHERE_LANG`（`zh` / `en` / `ru`）指定。
...
/usr/local/libexec/nowhere-launch                      # root 所属启动器（读取 url.conf）

/etc/nowhere/
├── url.conf                              # 运行 URL（权限 640，root:nowhere）
├── manager.conf                          # 管理元数据（权限 600，仅 root）
└── tls/
...
```bash
sudo bash nowhere-v1.sh install --method release --libc musl [其他参数...]
```
...
```bash
sudo bash nowhere-v1.sh install --method source --swap 4096 --jobs 1 [其他参数...]
```
...
* **解决**：务必先执行 `sudo bash nowhere-v1.sh prepare-tls --cert ... --tls-key ...`，并使用该命令输出的 `/etc/nowhere/tls/` 路径作为参数。
